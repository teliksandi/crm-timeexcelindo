...
class nonce {
    ...
    function ft_nonce_create_query_string( $action = '' , $user = '' ){
    	return $this->ft_nonce_create( $action , $user );
    }

    // This method creates an nonce for a form field
    function ft_nonce_create_form_input( $action = '' , $user='' ){
    	echo "<input type='hidden' name='".FT_NONCE_KEY."' value='".$this->ft_nonce_create( $action . $user )."' />";
    }
    ...
    function ft_nonce_create( $action = '' , $user='' ){
    	return substr( $this->ft_nonce_generate_hash( $action . $user ), -12, 10);
    }

    // This method validates an nonce
    function ft_nonce_is_valid( $nonce , $action = '' , $user='' ){
    	// Nonce generated 0-12 hours ago
    	if ( substr($this->ft_nonce_generate_hash( $action . $user ), -12, 10) == $nonce ){
    		return true;
    	}
    	return false;
    }
