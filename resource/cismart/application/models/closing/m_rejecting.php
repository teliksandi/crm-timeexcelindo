...
class m_rejecting extends CI_Model{
    ...
    function get_list_closing($filter, $params){
        $sql = "SELECT closing.id_initiation AS 'id_initiation', closing.id_closing, initiation.project_title, initiation.budget, client.client_name
                        ...
                        where closing.id_monitoring is NULL
    ...
    function get_total_closing($params){
        $sql = "SELECT COUNT(*) as 'total' FROM initiation WHERE project_title LIKE ? and id_monitoring is NULL";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
    ...
    function search_closing($filter, $params){
        $sql = "SELECT COUNT(*) as 'total', client.client_name, initiation.project_title, closing.id_closing, initiation.budget From closing left join initiation on initiation.id_initiation = closing.id_initiation  left join client on initiation.id_client = client.id_client where closing.id_monitoring is NULL and $filter like ? ORDER BY closing.id_closing DESC";
        $query = $this->db->query($sql,$params);
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
            $query->free_result();
   ...
   function get_koment(){
    ...
    function initiation_detail($where){
        $sql = "SELECT a.id_initiation, b.start_date as 'mulai', b.due_date as 'akhir', b.*, c.*, d.*, e.* FROM closing a 
                ...
                WHERE  b.id_initiation = ? and a.id_monitoring is NULL";
