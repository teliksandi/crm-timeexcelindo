...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^tanggal 20 november 2017^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    function get_list_monitoring($filter, $params, $id_department, $id_k){
        ...
        //^^^^^^^^^^ 22 november 2017^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        $sql = "SELECT monitoring.id_monitoring AS 'id_monitoring', initiation.project_title, initiation.budget, monitoring.due_date, initiation.id_initiation, monitoring.start_date, client.client_name
                        From monitoring left join execution on monitoring.id_execution = execution.id_execution
