...
class account_settings extends ApplicationBase {
    ...
    public function __construct() {
        ...
        parent::__construct();
        // load model
        $this->load->model('master/m_operator');
        // load library
    ...
    public function data_pribadi() {
        ...
        $this->smarty->assign("template_content", "operator/account_settings/index.html");
        // load javascript
        $this->smarty->load_javascript("resource/js/jquery/jquery-ui-1.9.2.custom.min.js");
        // load style ui
        $this->smarty->load_style("jquery.ui/redmond/jquery-ui-1.8.13.custom.css");
        // get data pribadi
        $result = $this->m_account->get_data_operator($this->com_user['user_id']);
        // images
    ...
    public function process_data_pribadi() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            $params = array(
                "operator_gender" => $this->input->post('operator_gender'),
                "operator_birth_place" => $this->input->post('operator_birth_place'),
                "operator_birth_day" => $this->input->post('operator_birth_day'),
            ...
            // update
            if ($this->m_operator->update_data_pribadi($params, $where)) {
                // upload foto
        ...
        // default redirect
        redirect("operator/account_settings/data_pribadi");
    }
    ...
    public function user_account() {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "operator/account_settings/account.html");
        // get data pribadi
    ...
    public function process_update_account() {
        ...
        // process
        if ($this->tnotification->run() !== FALSE) {
            $params = array($this->input->post('user_name'), $this->input->post('user_pass_new'), $this->com_user['user_id']);
            // update
        ...
        // default redirect
        redirect("operator/account_settings/user_account");
    }
    ...
    public function change_role() {
        ...
        $this->_set_page_rule("R");
        // set template content
        $this->smarty->assign("template_content", "operator/account_settings/role.html");
        // get data role
        $this->smarty->assign("rs_roles", $this->m_account->get_all_roles_by_users(array(2, $this->com_user['user_id'])));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // output
    ...
    public function process_update_role() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $this->tsession->set_userdata('session_operator', array('user_id' => $session['user_id'], 'role_id' => $this->input->post('role_id')));
            // notification
            $this->tnotification->delete_last_field();
            $this->tnotification->sent_notification("success", "Data berhasil disimpan");
        } else {
        ...
        // default redirect
        redirect("operator/account_settings/change_role");
    }
