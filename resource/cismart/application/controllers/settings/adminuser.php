...
class adminuser extends ApplicationBase {
    ...
    public function add_process() {
        // cek input
        $this->tnotification->set_rules('nama_lengkap', 'Nama Lengkap', 'trim|required|max_length[50]');
        $this->tnotification->set_rules('alamat', 'Alamat', 'trim|required|max_length[255]');
        $this->tnotification->set_rules('no_telp', 'Nomor Telepon', 'trim|required|max_length[50]');
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_settings->insert_user($params)) {
                ...
                // insert to users
                $params = array($user_id, $this->input->post('nama_lengkap'), $this->input->post('alamat'), $this->input->post('no_telp'), $this->com_user['user_id']);
                $this->m_settings->insert_user_detail($params);
                // insert hak akses
    ...
    public function edit_process() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_settings->update_user($params)) {
                // update to users
                $params = array($this->input->post('nama_lengkap'), $this->input->post('alamat'), $this->input->post('no_telp'), $this->com_user['user_id'], $this->input->post('user_id'));
